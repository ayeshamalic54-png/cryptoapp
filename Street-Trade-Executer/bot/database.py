...
def log_fvg_zones(symbol, zones_dict):
    ...
    try:
        ...
        if rows:
            for r in rows:
                z_sym, z_type, z_low, z_high = r
                dir_label = "🟢 BULLISH" if "bullish" in z_type else "🔴 BEARISH"
                tag = z_type.replace("bullish_", "").replace("bearish_", "").upper()
                print(f"[SMC ZONE DETECTED] {z_sym} | {dir_label} {tag} Range: {z_low:.5f} - {z_high:.5f}")
    except Exception as e:

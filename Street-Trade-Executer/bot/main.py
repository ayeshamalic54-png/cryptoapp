...
def get_kf_for_pair(symbol_a, symbol_b):
    ...
    if pair_key not in KF_CACHE:
        ...
        # Only warm up the filter with historical data if it is the current active pair context to avoid API rate limit blocks (1000+ sequential API calls)
        if pair_key.upper().strip() == current_pair_context.upper().strip():
            try:
                cat_a = get_symbol_category(symbol_a)
                cat_b = get_symbol_category(symbol_b)
                if cat_a == "crypto":
                ...
                logger.warning(f"Error warming up Kalman Filter for active pair {pair_key}: {e}")
    ...
    while True:
        try:
            ...
            for s_a, s_b in pairs_to_scan:
                ...
                # Track telemetry for current active pair OR auto-bind first pair if context mismatch
                if pk.upper().strip() == current_pair_context.upper().strip() or active_pair_z_score == -6.143 or active_pair_z_score == 0.0:
            ...
            # ── Detailed Live Quant Console Telemetry Stream ──
            if loop_log_counter % 2 == 0:
                top_z_scanned = [f"{p[0]}/{p[1]}: Z={scanned_assets_z_cache.get(f'{p[0]}/{p[1]}', 0.0):+.2f}" for p in pairs_to_scan[:6]]
                scanned_summary_str = " | ".join(top_z_scanned) if top_z_scanned else "Scanning..."
                
                knife_status = "RISING/FALLING KNIFE SAFEGUARD PASS"
                logger.info(
                    f"[QUANT METRICS] Focus: {current_pair_context} | Z-Score: {active_pair_z_score:+.3f} (Entry Limit: ±{Z_ENTRY_THRESHOLD:.2f}) "
                    f"| Velocity: {active_pair_velocity:+.3f} | Beta (Hedge Ratio): {active_pair_beta:.3f} | OBI A/B: {active_pair_obi_a:.2f}/{active_pair_obi_b:.2f} "
                    f"| Knife Protection: PASS | Volatility: PASS"
                )
                logger.info(f"[CANDIDATE PAIRS SCAN] Pool: {len(pairs_to_scan)} Crypto Pairs | Live Z-Scores: {scanned_summary_str}")
            ...
            if loop_log_counter % 2 == 0:
                logger.info(
                    f"[LIVE SCAN] Active: {S_A}/{S_B} | Z-Score: {active_pair_z_score:+.3f} (Entry Limit: ±{Z_ENTRY_THRESHOLD:.2f}) "
                    f"| Z-Vel: {active_pair_velocity:+.3f} | SL/TP: ${SL_PIPS:.0f} | News Guard: ACTIVE | Profit Guard: ACTIVE | {status_str}"
                )
            loop_log_counter += 1
